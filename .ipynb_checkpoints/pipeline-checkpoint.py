...
def fetch_job_details(session, job_id: str) -> str:
    """Fetches the full description using LinkedIn's public job card endpoint."""
    ...
    while total_saved < limit:
        ...
        try:
            ...
            for card in cards:
                ...
                # Skip if we already stored this posting
                with sqlite3.connect(DB_PATH) as conn:
                    exists = conn.cursor().execute("SELECT 1 FROM jobs WHERE job_id = ?", (job_id,)).fetchone()
                    if exists:
                        continue

                # Fetch full description for skill analysis
                desc_text = fetch_job_details(session, job_id)
                full_text_for_skills = f"{title_el.text} {desc_text}"
                ...
                job_data = (
                    ...
                    extract_skills(full_text_for_skills),
                    desc_text[:1000],  # Store truncated preview for performance
                    raw_url,
                    datetime.now()
                )
